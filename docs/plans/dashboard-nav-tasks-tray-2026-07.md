...
2. **Stage 2 — Global tray:** ✅ BUILT (awaiting deploy). A3 (`TodayTasksTray` — a right-anchored drawer on a new **`ModalShell` `drawer` variant** + header trigger with overdue badge + quick-add + board link) · B2 (relative due-date labels, extracted to the pure `src/lib/taskDueLabel.ts` helper) · B6 (NextBestAction rung wired from `TaskEngine.peekCache`). Files: new `src/components/TodayTasksTray.tsx` (placed at components root beside MobileNavDrawer/CaptureSheet, not `tasks/`), new `src/lib/taskDueLabel.ts`, `ui/ModalShell.tsx` (drawer variant), `src/App.tsx`, `TaskList.tsx`, `home/HomeMain.tsx`. Used `Z.drawer` (no new z constant needed). Tests: `taskDueLabel.test.ts` (10), `today-tasks-tray.spec.ts` (TRAY-001..004/010); NextBestAction's existing unit test already covers the rung. Docs: new `09-persistent-ui/12-today-tasks-tray.md` + `00-INDEX` row, `01-header.md`, `17-home-main.md`, `18-next-best-action.md`, e2e-test-plan §34 + index, TESTING.md. Validated: typecheck + build + unit 1570 green.
3. **Stage 3 — Task-row quality:** ✅ BUILT (awaiting deploy). B1 (empty states — "All done" vs "Nothing on the list", compact size='sm' chrome='none'; decision extracted to pure `src/lib/taskListEmptyState.ts`) · B3 (card states + 6 meaning chips → `status-*` tokens; type icons left as type-coding) · B4 (44px `pointer-coarse:` on complete toggle, select-all, 2 hero chips, add-location chip) · B9 (row "Recurring" pill + TaskModal recurrence row) · B14 (both `window.confirm` → in-app `ConfirmModal` via a promise-based `askConfirm`; Escape ref-guard). Files: `TaskList.tsx`, `TaskModal.tsx`, `AddTaskModal.tsx`, `HomeStatusStrip.tsx`, `GardenOverviewGrid.tsx`, new `src/lib/taskListEmptyState.ts`. Tests: `taskListEmptyState.test.ts` (3), `schedule.spec` SCH-B14. Docs: `01-add-task-modal.md`, `02-task-modal.md`, `34-accessibility.md`, e2e-plan §07, TESTING.md. Validated: typecheck + build + unit 1573 green.
4. **Stage 4 — Discoverability + errors:** ✅ BUILT (awaiting deploy). B5 (Ailment Library → ToolsHub tile + launcher pin) · B7 (Shelf drops `dashboard`/`shed`/`planner` at the App.tsx call site) · B8 (removed the no-op "Getting Started" dropdown item + its `Rocket` import) · B12 (Routines tab in PlannerHub → BlueprintManager) · B13 (Automations load-error card + retry when no cache) · B15 (Calendar "Operational Hub" → live "N tasks · M overdue" summary). Files: `ToolsHub.tsx`, `quickLauncherCatalogue.ts`, `src/App.tsx`, `UserProfileDropdown.tsx`, `PlannerHub.tsx`, `integrations/AutomationsSection.tsx`, `TaskCalendar.tsx`. Tests: `stage4-discoverability.spec` (DISC-B5/B7/B8/B12/B15). Docs: tools-hub, blueprint-manager, user-profile-dropdown, bottom-tab-bar, calendar-tab, integrations-automations, e2e-plan §35, TESTING.md. Validated: typecheck + build + unit 1573 green.
   - **B16 (Garden Reports): user chose SURFACE IT (2026-07-21).** Built as **Stage 5**: `/reports` route (lazy `GardenReports`, no new backend — the hook computes month/year stats client-side from `tasks`/`inventory_items`/`yield_records`/`weather_alerts`), a Tools-hub *Measure & Track* tile (`tools-hub-garden-reports`), a pinnable "Reports" launcher entry, and `/reports` + `/ailment-library` added to the Tools nav `matchPaths`. New reference file `docs/app-reference/05-tools/11-garden-reports.md` + `00-INDEX` row; DISC-B16 e2e green live.
