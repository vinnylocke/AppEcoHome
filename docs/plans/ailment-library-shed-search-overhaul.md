...
- ~~**Hearts at search time**~~ — **DESCOPED (Stage 4 build finding, 2026-07-21):** favouriting a library search result requires cloning it into the catalogue, where it becomes a `plants` row with `source='ai'` — and the plants-side favourite tier-trigger re-derives from `plants.source`, so free tiers would be blocked from hearting free library content: the exact defect class Stage 1's migration fixed for ailments, but on the plants side it's encoded in the client lock (`isSourceLockedForTier`), the server trigger, the docs matrix AND `plants_source_check` (no `'library'` value). Making "library = free" true for plant favourites is its own product decision + migration — flagged as a follow-up, not smuggled into a UI stage.
...
| **2 — Full-screen plant search** | ✅ BUILT (awaiting deploy). The takeover replaces BulkSearchModal as the Shed front door via an EARLY RETURN in TheShed (`handleProceedToBulkAdd`'s first line closes it, so the import-progress modal renders normally). Extraction preserved every contract: cart shapes + `preloadedDetails`, paste-a-list, `bulk-search-tab-*` (tour anchors — **no flowRegistry change needed**, copy was already flow-agnostic and SHED-017 proves the anchors live), `bulk-search-review`/`start-import`, `initialCartItems`→review (autoImport), setPageContext lifecycle, deep links + `state.returnTo`. Added: real Escape handling (review→search→close; guarded on the Manual form + under PlantDetailModal — the tray lesson), grid scroll restore, status-token/44px re-clothing. BulkSearchModal remains solely as CompanionPlantsTab's host. Tests: shed-crud 43/43 live (SHED-016's Escape is now genuine) incl. new SHED-TKO-001/002; shed-discovery + plants green. Docs: the-shed.md, 04-bulk-search-modal.md (role change), 36-plant-search.md (+ the two drift fixes: retired `/library` refs, 5-tab detail), e2e-plan §06, TESTING.md | new `shed/PlantSearchTakeover.tsx`, `TheShed.tsx` |
| **3 — Search-first landing** | ✅ BUILT (awaiting deploy). Hero search (full-width focal, 52px, shadow-card; aria-label preserved) · escalation row (`shed-search-library-escalation`: home-scope query ≥3 chars with ≤2 own matches → one tap into the takeover with the query) · persona browse chips (`shed-browse-chip-*`, new-gardener + <12 active + empty search → Edible favourites / Indoor friends / Sun lovers / Easy annuals; **`PlantSearch` gained `initialFilters`** (mount-seeded, panel auto-opens) threaded via the takeover) · blur consolidation (toolbar now solid — GardenHub's bar is the one blur) · SegmentedTabs adoption for the view/scope/status toggles (**the primitive gained per-tab `testId`** so every load-bearing selector survives, and a **measure() bail-out fixing a live-caught "Maximum update depth exceeded" crash** — new-object setState per run + unstable tabs-array deps compounded under the assignment modal's re-render storm; tabs arrays also memoised). Tests: SHED-S3-001/002 green (S3-002 self-skips on a dirty ≥12-plant DB), SHED-031/033 crash regression fixed + green, full shed-crud/journal/layout suites re-run | `TheShed.tsx`, `shared/PlantSearch.tsx`, `shed/PlantSearchTakeover.tsx`, `ui/SegmentedTabs.tsx` |
| **4 — Favourites act + craft** | ✅ BUILT (awaiting deploy). **"Add & assign…"** leads every not-in-home favourite card (`favourite-add-assign-{id}`): copy via `addFavouritePlantToHome` → scope jumps to Home → `PlantAssignmentModal` opens on the fresh row (full areas/tasks/smart-schedules pipeline); copy-only add is the compact secondary (testid preserved). Repairs: **bulk assign now calls `applyPlantedAutomations`** (insert select gained `home_id` — the engine reads it off item[0]) + **Verdantly** source-filter option. Craft: 44px `pointer-coarse:` on card heart/kebab, bulk-assign steppers, favourites action buttons + `can-hover` guards. **Hearts-at-search DESCOPED** (see B3 note — plants-side library-tier product call). Tests: SHED-FAV-001 green live (self-cleaning) | `FavouritePlantsGrid.tsx`, `TheShed.tsx`, `BulkAssignModal.tsx` |
...
✅ BUILT (validating). Executed as an **in-place shell conversion** (not an extraction — minimal churn in the 2,280-line file): `AddAilmentModal`'s `fixed inset-0` portal shell + `max-w-3xl h-[85vh]` panel + `backdrop-blur-xl` became a page block (`ailment-add-takeover`) with an `ailment-add-back` "Watchlist" control; `AilmentWatchlist` early-returns it while `showAdd` (all hooks above; the old `createPortal` render removed). Every internal testid preserved (`ailment-tab-*`, `ailment-search-*`, `ailment-library-*`, `ailment-back-to-search` — watchlist.spec's WL-MOBILE-002/WL-MODAL-003 drive them); `?open=add-ailment` + `ailment_source` preference + `existingKeys` Watching-states unchanged. Guarded Escape added (review → tabs → library search → close; never on Manual). No flowRegistry anchors exist for this flow (verified). Files: `AilmentWatchlist.tsx` only.
