...
- **Stage 4 — Locations consolidation.** Split into **4a** (retire tab) + **4b** (inline manage) for safe, independently-shippable slices.
  ...
  - **Stage 4b — Inline add/manage on the grid (NEXT).** Extract a shared `AddLocationForm` (+ the create/rename/env/delete mutations from LocationManager 227-378, which all already `onDataChanged?.()`); add `home-add-location-btn` at the grid section header + a per-card manage kebab (rename / inside-outside / delete) — **all `can()`-gated** (member = create+edit not delete; viewer = none). Net-new plumbing flagged by recon: `usePermissions()` into the grid/card (absent today) + a refresh callback App→HomeMain→grid→card (mirror LocationManager's `onDataChanged` → `handleHomeDataRealtime`). **New permission e2e** covering the split.
