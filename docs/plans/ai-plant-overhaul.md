...
**Second sub-fix:** the catalogue inserts in `plant-doctor` were failing with `null value in column "id" of relation "plants" violates not-null constraint`. `plants.id` is `integer NOT NULL` with **no database default** — every insert must supply an id explicitly. The seed data + client-side inserts (TheShed bulk-add, PlantSearchModal) all do `Math.floor(Date.now() / 1000) + Math.floor(Math.random() * 1000)`, but the edge function inherited that schema assumption without setting the id. The race-recovery path silently swallowed the failure → `dbPlantId` stayed null → all AI plants ended up orphan. Both catalogue-write sites in `plant-doctor` (the new step-2 legacy-cache self-heal AND the existing step-4 fresh-Gemini path) now supply an id with the same convention. Verified end-to-end with a manual `generate_care_guide('Pot Marigold')` call — global inserted at id `1779284168`, `db_plant_id` returned in the response, `fromCatalogue: true` on subsequent calls.

**Third sub-fix — configurable refresh cadence.** The 7-day `manual-refresh-ai-plant` rate limit was painful for local testing. Made it env-configurable via `AI_REFRESH_RATE_LIMIT_MINUTES` declared in `supabase/config.toml` `[edge_runtime.secrets]` and sourced from `supabase/.env`. Local dev sets it to `1` (one minute); prod default stays at 7 days when unset. The brittle client-side 7-day `localStorage` cache was removed entirely — the server is the truth, the `refreshing` button state handles in-flight click suppression, and the rate-limited response carries `rate_limit_minutes` + `retry_after` for any UI that wants to show specifics later. Verified end-to-end: a seeded refresh log 30s ago returns `rate_limited` with `rate_limit_minutes: 1`; a refresh log 65s ago passes through.

**Fourth sub-fix — catalogue lookup-by-common-name + better rate-limit toast.** Diagnosed why Pot Marigold kept failing while Rhubarb worked: the catalogue's unique index is on `scientific_name_key`, but `plant-doctor.generate_care_guide` looks up by a key derived from the *common name* the user typed. When the global already exists keyed by scientific name (e.g. "calendula officinalis" for Pot Marigold), the common-name lookup misses → INSERT attempt → unique-index conflict → race-recovery re-reads with the wrong key → orphan persists. Two-part fix to `supabase/functions/plant-doctor/index.ts`:

1. **Step 1 catalogue read** now falls back to `common_name ILIKE cleanName` when the `scientific_name_key` lookup misses. Catches the common case where the user types a common name and the global was keyed by its scientific name.
2. **Race-recovery in both the legacy-cache self-heal (step 2) and fresh-Gemini paths (step 4)** now compute the actual key from the insert payload's scientific_name (which is what the unique index sees) and re-read with BOTH that key AND the original lookup key, plus a final `common_name ILIKE` fallback. The race-recovery now reliably resolves to the existing global instead of returning null.

Verified end-to-end: a fresh `generate_care_guide('Pot Marigold')` call now returns `db_plant_id: 1779284168` (the existing global at scientific_name_key="calendula officinalis") with `fromCatalogue: true`. The user's orphan home Pot Marigold will heal on its next Refresh click.
