...
- **Phase 5** — **task views render offline + offline routines.** Gave `TaskEngine` the persistent snapshot it lacked (`rhozly:snap:v1:tasks:{homeId}`, raw inputs), extracting the ghost/dedup logic into a pure `buildRenderTasks` shared by the online and offline paths (guarded byte-identical by the existing 41 engine tests). Offline-created one-off tasks now inject into that snapshot so they appear in **every** task view instantly; **new recurring routines** are creatable offline too (blueprint + first task queued with client uuids, ghosts render the recurrence, `generate-tasks` skipped until reconnect). Verified end-to-end against a real DB (offline create one-off + routine → visible via the engine → reconnect → blueprint + both tasks persisted with the FK intact); +10 unit tests. See [`offline-tasks-and-routines.md`](./offline-tasks-and-routines.md).

- **Phase 5 follow-up** — **routine editing + dependency linking offline.** The last two gated paths in `AddTaskModal` now work offline: editing a routine is an `updateOrQueue("task_blueprints")` + re-inject (inject made replace-by-id) so ghosts regenerate from the new values; linking a dependency materialises a ghost target offline (client-uuid task insert) then queues the `task_dependencies` row. Verified end-to-end vs a real DB (offline edit changed ghost cadence; offline link + materialise → reconnect → blueprint updated + dependency + materialised task persisted, FKs intact). See [`offline-routine-edit-and-linking.md`](./offline-routine-edit-and-linking.md). **AddTaskModal is now fully offline-capable.**
