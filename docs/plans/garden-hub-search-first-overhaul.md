...
4. **Nursery promotion** — ✅ **BUILT 2026-07-21.** 4th hub tab `garden-hub-tab-nursery` (order Plants|Watchlist|Nursery|Senescence — Shepherd-anchored tabs stay slots 1–2); `?tab=nursery` deep link; TheShed's Plants|Nursery toggle + `view` state DELETED (shed-view-* testids retired; NurseryPage PO retargeted to the hub tab + `?tab=nursery` goto); NurseryTab gets the shared HubHeader in a new `searchMode="input"` variant (REAL inline search `nursery-search-input` — local data, filters variety/plant/vendor live) + a single "Add seeds" primary (`nursery-add-seeds-btn`) opening a bottom action sheet (Scan/Paste/Type — the old `nursery-scan/paste/add-packets` testids moved onto sheet rows; the 3-button toolbar cluster + old scope pills died); All/♥Favourites chip row keeps `nursery-scope-*` testids; `nursery_tour` copy updated. NurseryTab now renders bare under GardenHub (App provides padding — same as the watchlist), which also retires the S3-deferred padding question: each tab is consistent within the hub.
5. **Weather strip collapse** — ✅ **BUILT 2026-07-21** (owner Q1 locked app-wide). `WeatherAlertBanner` compact mode: N≥2 alerts collapse into ONE 44px strip (`weather-alert-strip`, worst-severity styling + headline, ⚠ + chevron) expanding IN PLACE to the classic per-type rows (`weather-alert-strip-collapse` to re-collapse; per-type dismissal logic untouched; single alert renders unchanged; App.tsx untouched — the collapse lives inside the component). Measured: the Shed launcher rose y=430 → y=323 (~107px back on every padded screen). e2e: DASH-009b added (strip → expand → collapse round-trip); DASH-010/011/013 gained an expand-first step.
