...
2. **Ailment takeover parity** — same shell; tappable rows → extracted `AilmentDetailContent` modal; instant-add +. Files: `AilmentWatchlist.tsx`, `AilmentLibrary.tsx` (+ new shared detail component).
3. **Landing chrome diet** — `HubHeader` (new), Plants + Watchlist landings, Filters bottom sheet, GardenHub padding/z ownership, Senescence 44px fix, tour copy update for the search-led anchor. Files: `garden/HubHeader.tsx`, `GardenHub.tsx`, `TheShed.tsx`, `AilmentWatchlist.tsx`, `garden/SenescenceTab.tsx`, `onboarding/flowRegistry.ts`.
