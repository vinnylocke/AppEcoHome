...
| CHAT-015 | 🔲 | Assistant reply with `**bold**` + a `- list` renders as formatted markdown (`react-markdown`); the rendered text contains no literal `*`/`#` markers. User bubbles stay plain text. | `agent-chat` (markdown reply) | 🔲 Planned |
| CHAT-016 | 🔲 | Reopening a thread with >50 stored messages shows the **newest** turns (not the oldest 50) — regression guard for the ascending+limit(50) history bug | seed >50 `chat_messages` | 🔲 Planned |
| CHAT-017 | 🔲 | Read-aloud strips markdown before TTS — text sent to `tts-speak` has no `*`/`#`/`- ` markers (`markdownToSpeech` covered directly by `stripMarkdownImages.test.ts`) | `tts-speak` spy | 🔲 Planned |
