...
| WALK-027 | ✅ | Repeat walks: complete the entire walk (`completeEntireWalk` helper) → summary's **Start a full walk** (`walk-summary-full-walk`) produces a real second route; abandon it, re-enter → empty screen shows **Walk everything again** (`garden-walk-empty-again`) which starts a real walk (fresh mode bypasses the same-day filter) | `home-overview` | ✅ Passing (self-skips if no walkable route) |
