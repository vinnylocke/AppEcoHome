...
| DOC-017 | Multi-ID — select + confirm a plant | ⬜ | In a region, click `scene-map-candidate-0-1` → `scene-map-confirm-0` → `scene-map-confirmed-0` shows the selected candidate's name | Bootstrap | `plant-doctor` identify_scene | ⬜ Planned |
