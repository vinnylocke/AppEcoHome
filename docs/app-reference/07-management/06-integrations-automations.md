...
- **Actions** — an ordered `automation_actions` list. Three kinds:
  - `send_notification` — **Send notification** (UI label): a custom title + body **reminder** sent to every `home_member` when the automation **fires** (conditions met). Turns an automation into a reminder. Handled by `fanoutActions` on the firing edge; its distinct-member reach feeds the run's `members_alerted`.
  - `notification` — the **Automation Receipt** (UI label, 2026-06). Opt-in: when an automation has this action, the **runner** sends every `home_member` a **system-generated outcome** notification (ran · held back by rate limit · failed) at each meaningful run-decision — *not* a custom message, and never on idle "not due" checks (which would spam every 5–15 min). Built by `_shared/automationReceipt.ts` (`buildReceipt` pure + `sendReceipt`, which self-gates on the action's presence and fans out per member, returning the count). It lives in the runner (not `fanoutActions`) so it can also fire on rate-limit/failure, not just on a successful fire. The old auto watering notifications (dead `sendNotification`) were removed; without either notification action the automation is silent.
  - `valve_open` — enqueues a `turn_on` on `automation_valve_queue`, **and (2026-06-18 fix) when `valve_duration_seconds` is set also enqueues the paired `turn_off` at `fire_at + duration`** so the valve actually closes after its run time. Both the auto fire (`evaluate-automations`, 2026-06-23) and the manual run drain the just-queued `turn_on` **inline** via the shared `_shared/valveQueue.ts` (`drainValveQueue`) so the valve actuates immediately; the **every-minute** (2026-06-25) `drainValveQueue` step in the `run-automations` cron — each entry **claim-locked** so the inline + cron drains can't double-fire — closes the paired `turn_off` within ~1 min of its `fire_at`. The auto-off **countdown sent to the device is the action's `valve_duration_seconds`** (what the user set), not the legacy `automations.duration_seconds` default. Row-building is the pure `_shared/valveQueueRows.ts` (`buildValveQueueRows`, Deno-tested). Previously the engine enqueued only the `turn_on`, so valves stayed open indefinitely.
...
Each run writes an `automation_runs` row; the card shows the last-run status pill. `devices_triggered` stores `members_alerted` (how many members the receipt reached); the run-history summary (`src/lib/automationRunSummary.ts`) renders "**N members alerted**" (legacy rows stored the same per-member count under `notifications`, so the wording is accurate for them too).
...
- **Run limit** — `automations.run_limit_count` per `run_limit_window_hours` (NULL = unlimited). Before firing, the engine fetches the in-window fired runs (`FIRED_STATUSES`, newest-first, `_shared/runLimit.ts`); when over the limit it stores the exact next-eligible time in `automations.rate_limited_until` and **mutes re-evaluation until then** (see "Run-status visibility" below) — no per-tick work. The card shows a "≤ N/Hh" chip. Note this is a rolling-window **cap**, not a scheduler: with repeat-while-true firing an automation fires up to the cap (≈ a cooldown apart) then waits until the earliest of those fires ages out of the window — which is exactly the `rate_limited_until` instant.
...
### "Ran" receipt fired ~5 min before the valve opened (2026-06-23 fix)
...
- `src/components/integrations/AutomationSuggestions.tsx` — **tuning suggestion chip (Pillar B):** reads active `automation_suggestions`, one-tap Apply (mutates the automation, gated by automations RLS) / Dismiss / Details. Generated by `analyse-automations` from run history + the soil-moisture model. See [Data Model — Integrations](../99-cross-cutting/09-data-model-integrations.md) + the [automation-intelligence plan](../../plans/automation-intelligence-and-soil-drydown.md).
...
- `supabase/functions/analyse-automations/index.ts` — suggestion generator (daily cron + on-demand)
