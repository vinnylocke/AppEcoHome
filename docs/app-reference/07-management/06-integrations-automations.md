...
- **Actions** — an ordered `automation_actions` list. Three kinds:
  ...
  - `valve_open` — enqueues a `turn_on` on `automation_valve_queue`, **and (2026-06-18 fix) when `valve_duration_seconds` is set also enqueues the paired `turn_off` at `fire_at + duration`** so the valve actually closes after its run time. Both the auto fire (`evaluate-automations`, 2026-06-23) and the manual run drain the just-queued `turn_on` **inline** via the shared `_shared/valveQueue.ts` (`drainValveQueue`) so the valve actuates immediately; the every-5-min `drainValveQueue` step in the `run-automations` cron remains the safety net and closes the paired `turn_off` when its `fire_at` arrives. Row-building is the pure `_shared/valveQueueRows.ts` (`buildValveQueueRows`, Deno-tested). Previously the engine enqueued only the `turn_on`, so valves stayed open indefinitely.
