...
**Route:** `/walk` (focus-mode shell on mobile — no top bar, no side nav; renders inside the shell on desktop, with the Plant Doctor chat FAB suppressed)
...
- `src/components/walk/WalkPlantCard.tsx` — one full-bleed card per plant + sticky action bar + its task rows
...
│   │   │   └── Log reading → WalkReadingSheet → areaReadingsService.logManualReading
...
| Plant outcomes | `INSERT garden_walk_visits (inventory_item_id, outcome)` — unchanged |
...
| Harvest "Picked some" (Phase 3) | `HarvestPartialPickSheet` (the Task Detail component) → `yieldService.insertYieldRecord` — one `yield_records` row per linked instance — then `taskActions.snoozeHarvestTask(task, days)` |
...
- [Routing](../99-cross-cutting/21-routing.md) — `/walk` joins the focus-mode shell on mobile.
...
- `src/components/walk/WalkReadingSheet.tsx` — manual reading capture (stamped now) over `areaReadingsService.logManualReading`; persona helper copy
