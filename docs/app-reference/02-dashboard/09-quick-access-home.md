...
> - **The customisable launcher** → was moved to the dashboard's `QuickActionsRow`, but **that launcher grid was itself removed from the home in the dashboard-nav-tasks-tray redesign Stage 1 (2026-07-21)** — every tile but the Garden Walk duplicated a nav-bar destination. The pin catalogue + the `/gardener?section=quick-launcher` picker survive in code; nothing renders those pins on the home now. `QuickActionsRow` is now the Garden-Walk-tile-only component. See [Home (Main Dashboard)](./17-home-main.md).
> - **The planting helper (the "Today" tile)** → `/quick/calendar` (`LocalizedTaskCalendar`) is KEPT as a focus-mode tool. It used to be reached via the dashboard's "Today" launcher tile (`home-quick-tile-today`), but **that tile was removed with the launcher grid in Stage 1 (2026-07-21) — the helper is now URL-only** (`/quick/calendar`), pending a re-surface. Its back button returns to `/dashboard`. See [Localized Task Calendar](./10-localized-task-calendar.md).
