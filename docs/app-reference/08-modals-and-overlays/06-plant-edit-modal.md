...
- `manual-refresh-ai-plant` — "Refresh care guide" button on the Care tab (Sage+ only). A pure DB sync: NEVER calls Gemini (the `refresh-stale-ai-plants` cron is the only content producer) — it diffs the home row against the global catalogue row and applies any pending values, or no-ops with "up to date". **Unlimited** since 2026-07-08 (rate limit disabled by default; `AI_REFRESH_RATE_LIMIT_MINUTES` re-enables a cadence).
...
- **AI freshness callout (Wave 5; review-and-apply rework 2026-07-08)** — appears at the top of this tab when the plant is an AI catalogue entry whose version is ahead of your ack. Yellow banner showing each changed field with its **before → after** values (fetched from `plant_care_revisions.diff_summary` for the versions since your ack; omission-noise rows filtered), with two actions: **"Apply updates"** (syncs the catalogue's new values into this plant via `revert_ai_plant_fork_in_place`, acks, closes the modal so the parent refetches) and **"Keep mine"** (acks only; your data untouched). Resolves via `forked_from_plant_id` for shallow forks added through bulk-add — the chip's source of truth is always the global catalogue row. An `ai-fork-consequence-note` under the source area warns that editing creates a personal copy that stops auto-updating.
- **"Refresh care guide" button (Sage+)** — button to the right of the "catalogue updated N days ago" pill. Triggers `manual-refresh-ai-plant` (pure DB sync from the catalogue — never Gemini); on success a toast reports how many fields changed and the chip clears, or "up to date" when nothing differs. **No cooldown** — refresh as often as you like (2026-07-08). Hidden for custom forks since they've opted out of catalogue updates.
- **Post-refresh confirmation strip (2026-07-08)** — after a refresh that applied changes, a green `ai-refresh-applied-strip` states the updates are **already saved** ("no need to press save") with a **Done** button that closes the modal. Added because the only visible action after a refresh was the copy-on-write "Save as my own copy" submit — one accidental click away from forking the plant off auto-updates.
- **Pristine fork-save guard (2026-07-08)** — for non-manual plants, the "Save as my own copy" submit is **disabled until the user actually edits a field** (`disableWhenPristine` in `ManualPlantCreation`, with a "No changes to save" tooltip). Reading or refreshing a plant can no longer end in an accidental fork.
...
**Modal stays open after refresh.** When Refresh applies changes, the modal re-fetches the updated home plant row from the database, updates `fullPlantData` (so the form re-renders with the new top-level values), and keeps the changed fields highlighted via `lastRefreshChangedFields` — a local state that overrides the freshness hook's `updated_care_fields` for the lifetime of the modal session. Without this override, the highlights would disappear immediately because the ack catches up to the catalogue's version, so the freshness hook flips `has_update` to false. `lastRefreshChangedFields` clears when the modal switches to a different plant or closes.

**Rate limit configuration.** Disabled by default since 2026-07-08 (`RATE_LIMIT_MINUTES = 0` — refresh is a costless DB sync that no-ops when the catalogue hasn't changed). Setting `AI_REFRESH_RATE_LIMIT_MINUTES` to a positive minute count on the `manual-refresh-ai-plant` edge function re-enables a per-(user, plant) cadence — declared in `supabase/config.toml`'s `[edge_runtime.secrets]` block and sourced from `supabase/.env`. The `refreshing` button state during in-flight requests is the only client-side suppression.
