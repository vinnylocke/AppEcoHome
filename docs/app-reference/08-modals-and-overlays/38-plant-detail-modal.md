...
- **Variety-name preservation (2026-07-23).** `plant_library` / the AI catalogue is largely species-level, so a cultivar pick ("Radish 'French Breakfast'") resolves onto the species row ("Radish") and its `common_name` would collapse. The modal's displayed identity is now `preferSpecificName(result.common_name, plant.details.common_name)` (`src/lib/plantNames.ts`) — it keeps the picked name when it **extends** the catalogue name (`"<species> …"`), else uses the catalogue name. Applied to the title, the Care tab's name field, and the Grow / Companions / Light / Soil tabs. Display-only — the species clone still supplies the care DATA (correct for the cultivar); the shared catalogue rows are never mutated. Unit-tested in `tests/unit/lib/plantNames.test.ts`.
