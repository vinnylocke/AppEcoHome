...
| `selectedAilment` | Currently open OWNED-ailment detail — **derived from `?owned=<ailments.id>`** (not local state, #7), so open PUSHes a history entry / close REPLACE-deletes it, and swipe-back / Escape closes just the modal instead of popping the Plants→Ailments tab switch. The search-takeover library-row detail `detailAilment` (#7b) uses the same idea via a `?lib=<ailment_library.id>` param (open PUSHes, close REPLACE-deletes; the transient result object is stashed in a ref, the param is source-of-truth). |
...
## Search unification (Hub v3 Stage E, 2026-07-22)
...
- **`/ailment-library` deleted → redirect** (`AilmentLibraryRedirect`, `src/App.tsx`): `?ailment=X` carries over to `&detail=X`. Tools-hub tile retargets to `/shed?tab=watchlist` (the quick-launcher entry that also pointed there no longer exists — the Quick Launcher customiser was removed outright 2026-07-23); the takeover's idle "Browse the field guide" row and the header-menu entry were deleted (the search IS the browse).


## The watchlist rebrand + visibility law (2026-07-22, post-v3)

- **Page title is "Ailments"** (matches the hub tab). NO HEARTS anywhere on ailment surfaces (owner rule — negative connotation): the personal cross-home layer is the **🔭 Watchlist** (Binoculars icon everywhere).
- **Chips: All / Active / Inactive / Watchlist · N** — the "Watching" chip DIED; the merged Watchlist chip (testid `watchlist-scope-favourites` kept) opens the affinity grid. Visibility law: the **All** grid = presence OR 🔭; `watchlist-hidden-collection-hint` covers the hidden remainder. **Smart default (2026-07-23):** lands on the **Active** chip when live-linked ailments exist (derived via `plant_instance_ailments`), else stays on **All** — a one-shot on load that never overrides a manual chip pick. **Adding is watching:** every add path auto-🔭s via the page's `autoWatch` (onSaved chokepoint + bulk + `?detail=` watch).
- **Card parity with plants:** `ailment-card-{id}` + stagger; actions in the body row — 🔭 toggle (`watch-ailment-{id}`) + kebab (`ailment-card-kebab-{id}`: Archive/Delete) — never floating on the photo; source badge bottom-left, gallery bottom-right; grid `xl:grid-cols-4 gap-6`; HubHeader `bleed`.
- **Image judge / replace (2026-07-23):** a top-right corner button on the card image (`judge-image-ailment-{id}`, `stopPropagation`'d so it doesn't open the card) → the shared `JudgeImagePrompt`. **Wrong** records the current image in `image_rejections` (home-scoped, never re-served for this home) and fetches the next candidate via the NEW **`ailment-image-search`** function (Perenual → iNaturalist → Wikipedia + ailment-aware Gemini vet — plant search mis-scores pests; see [Edge Functions](../99-cross-cutting/10-edge-functions-catalogue.md)), then writes BOTH the home `ailments.thumbnail_url` (direct card render) AND the per-home `ailment_image_overrides` (source of truth for library/field-guide surfaces — `ailment_library` is global read-only) via [`src/lib/ailmentImageOverride.ts`](../../../src/lib/ailmentImageOverride.ts). When the card is **icon-only** (null image — the common library-add case) the same button reads **"Add a photo"** and runs the fetch directly (no reject). Empty pool → keep + "no other photos found" toast. (A detail-view `heroExtra` judge + a full ailment picker are a planned fast-follow; today the judge lives on the card.)
- **One watch verb:** the field-guide detail's "Add to watchlist" sets the home row AND the 🔭 layer in one tap (`AilmentDetailModal` wraps onWatch); the separate ♥ toggle (`ailment-detail-favourite`) DIED. Takeover library glyph: `ailment-library-watch-glyph-{id}` (Binoculars).
- Copy sweep: all "favourite(s)" strings → watchlist voice (cards, grid empty/hint states, bulk add "Mark all as watched", toasts, CSV template label — the CSV **header** stays `favourite` for back-compat).
