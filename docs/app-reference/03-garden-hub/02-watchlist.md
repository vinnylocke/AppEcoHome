...
- **Chips: All / Active / Inactive / Watchlist · N** — the "Watching" chip DIED; the merged Watchlist chip (testid `watchlist-scope-favourites` kept) opens the affinity grid. Visibility law: default grid = presence OR 🔭; `watchlist-hidden-collection-hint` covers the hidden remainder. **Adding is watching:** every add path auto-🔭s via the page's `autoWatch` (onSaved chokepoint + bulk + `?detail=` watch).
