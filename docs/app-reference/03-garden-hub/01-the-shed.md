...
- **Visibility = presence OR affinity.** The default (All) grid shows only plants that are Active/Inactive (derived) or ♥-favourited. The "Saved" chip DIED; the chip row is All / Active / Inactive / **♥ Favourites** (the merged cross-home affinity view, testid `shed-scope-favourites` kept). Zero-presence un-hearted rows are search-only; a quiet `shed-hidden-collection-hint` footer ("N more in your collection — search to find them") opens the takeover. **Adding is loving:** every add path (bulk import, manual, quick-add verbs) auto-♥s the new row via `savePlantToDB` (best-effort; tier-locked sources skip) so a fresh add never vanishes.
