...
The takeover's owned section is **"In your garden"** (was "In your Shed") and now includes **curated-out rows**: all home plants are matched (archived included), badge-sorted Active > Inactive > Saved > **Previously** (`data-presence="previously"` -- archived with no derived presence, dashed muted pill). Library/external result rows show a filled **heart glyph** (`{rowTestid}-fav-glyph`) when they match one of the user's favourite plants -- identity via `src/lib/libraryFavouriteMatch.ts` (ref ids + provider ids + the client mirror of `scientific_name_key`; `LIVE_PLANT_SELECT` widened to carry the bridges). The search detail modal (`PlantDetailModal`, via a new `actionsSlot` prop) gains the **three-verb footer**: **Plant it** (`plant-detail-plant-it` -- imports then auto-opens `PlantAssignmentModal`), **Sow seeds** (`plant-detail-sow-seeds` -- imports then opens the Seed box with a hint toast), **Save for later** (`plant-detail-save-later` -- imports only = Saved). Implementation: `PlantSearchTakeover.onQuickAdd` builds the single-item cart; TheShed awaits the normal import then resolves a `postImportIntent` (name-matched against the refreshed list, 2-minute staleness guard).


## The visibility law + feedback polish (2026-07-22, post-v3)

- **Visibility = presence OR affinity.** The **All** grid shows only plants that are Active/Inactive (derived) or ♥-favourited. The "Saved" chip DIED; the chip row is All / Active / Inactive / **♥ Favourites** (the merged cross-home affinity view, testid `shed-scope-favourites` kept). **Smart default (2026-07-23):** the Shed lands on the **Active** chip when the garden has live plants, else stays on **All** — a one-shot on load (fires once plants + presence have both loaded) that never overrides a manual chip pick (`didSmartDefaultRef` / `userChoseChipRef`). Zero-presence un-hearted rows are search-only; a quiet `shed-hidden-collection-hint` footer ("N more in your collection — search to find them") opens the takeover. **Adding is loving:** every add path (bulk import, manual, quick-add verbs) auto-♥s the new row via `savePlantToDB` (best-effort; tier-locked sources skip) so a fresh add never vanishes.
- **Instance counts are LIVE-only** (`useCachedShed` filters `ended_at IS NULL AND status <> 'Archived'` — the presence view's predicate); ended instances surface as the muted dashed **"N past"** chip (`plant-past-count-{id}`) and "· N past" in the takeover meta line.
- **History detail restored:** the plant modal's History rows are tappable (`plant-history-open-{id}`) → `InstanceEditModal` on the ended instance (amend natural/other, edit summary, the `analyse-plant-end-of-life` AI feedback, journal + closing photo, which also thumbnails the row).
- **Image judge / replace (2026-07-23):** the plant edit modal's Care-tab hero carries a "wrong photo?" button (`judge-image-plant-{id}`, beside the pencil) → a shared `JudgeImagePrompt` ("Is this the right photo of {name}?"). **Wrong** records the current image in `image_rejections` (home-scoped) via `src/lib/imageRejections.ts` so it's never re-served for this home, then fetches the next candidate through `plant-image-search` (rejection-aware — see [Image Sources](../99-cross-cutting/24-image-sources.md#rejection--per-home-image-override-2026-07-23)) and sets it into the form. **Nothing persists until Save**, so an AI/Perenual/Verdantly plant forks exactly as any other field edit (`handleSaveWithOverride` → `onForkSave`); the `WikiImagePicker` "Search Wikipedia" button remains the manual fallback. Empty pool → keep the current image + a "no other photos found" toast. (A card-grid judge overlay is a planned fast-follow; today the judge lives on the detail hero.)
