...
3. **`searchExternal` / `searchExternalPaged`** — Perenual + Verdantly via `searchAllProviders` / `searchAllProvidersPaged`. **Opt-in** ("Search more databases"). Since 2026-07-22 the component uses the paged variant: per-provider cursors (`ProviderCursor`) + an IntersectionObserver sentinel (`plant-search-external-sentinel`) load further provider pages seamlessly as the user scrolls, until both providers report no more (`cursorHasMore`). Appended rows dedupe by `provider:id`. The AI branch runs on the first page only.
...
- Result rows are 72px two-line rows (56px thumbnail, 16px name, then a **leading coloured source chip** + latin second line) with a separated 44px `+` add button per row (`{rowTestId}-add`). The chip (`{rowTestId}-source-badge`, 2026-07-22) gives every source a distinct colour — Library = house green, Perenual = sky, Verdantly = emerald, AI = amber — replacing the plain trailing provider word that was easy to overlook.
...
**Post-v3 polish (2026-07-22):** owned-row meta gains "· N past" (ended instances); `OwnedPlantMatch.past_instance_count`.

## Selection identity (multi-select hosts)

Multi-select hosts (PlantSearchTakeover, BulkSearchModal) key their cart by a
`selectionKey`: `per:{perenual_id}` / `ver:{verdantly_id}` / **`lib:{library_id}`
for library rows (2026-07-22)** / common name only for ai/manual (no stable id).
The library-id key is load-bearing: several library species share a common name
("Lavender" ×3 in the seeds), and the old name key made selecting one row light
up every namesake. `PlantFirstPlanForm` keys by name+scientific-name for the
same reason. Seeded regression fixture: plant_library 910002/910004/910005;
spec SHED-TKO-004.
