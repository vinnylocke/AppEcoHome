...
| `/library` search | ✅ migrated + unified — library hits route through the existing `plant_library_id` clone path; `allowPreview` adds the ⓘ peek, tap still navigates to the full preview page (no overlay). |
