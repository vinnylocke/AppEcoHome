...
**Write-back / self-healing (2026-06-18):** `area-sensor-analysis` now **persists the resolved ranges back onto the global `plants` row** wherever a column was null — covering BOTH library-sourced values (so the table is actually populated, no per-run library join next time) AND AI-generated values for plants the library hasn't seeded (generated via Gemini, `_shared/plantCareRangeGen.ts`, bounded to 3 plants/run, cache-miss only). Because `plants` is a shared catalogue, the ranges land **once for every user** and are reused thereafter. The insight uses the resolved ranges immediately. AI usage logged as `care_range_backfill`. (Earlier the write only happened for generated plants, so library-matched plants left `plants.soil_*` null — fixed.)
