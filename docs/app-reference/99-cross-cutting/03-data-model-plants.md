...
**Write-back / self-healing (2026-06-18):** when a plant is in **neither** `plants.soil_*` nor the library, `area-sensor-analysis` now **generates the ranges via Gemini and writes them to the global `plants` row** (`_shared/plantCareRangeGen.ts`; bounded to 3 plants/run, only on a cache-miss regeneration). Because `plants` is a shared catalogue, the ranges are generated **once for every user** and reused thereafter — closing the population gap for plants the library hasn't seeded yet. The same insight also uses the freshly-generated ranges immediately. AI usage logged as `care_range_backfill`.
