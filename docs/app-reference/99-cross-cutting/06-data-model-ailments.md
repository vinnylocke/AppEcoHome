...
> **Shape note (#11):** `treatment` / `prevention` are single **TEXT** columns (newline-joined step blobs from the generator's `stepText()`). The home `ailments` row instead stores `remedy_steps` / `prevention_steps` as jsonb **arrays**, and the watchlist counts discrete array elements — so collapsing the blob to one step made "N treatments" read 0–1. `splitStepsText()` (in `ailmentLibraryService.ts`) now splits the blob into discrete steps in `mapLibraryToWatchlistPayload` + `libraryRowToFavouriteInput` **and** the field-guide `AilmentDetailBody` render. No historical backfill — existing home rows keep their single collapsed step until re-added.
