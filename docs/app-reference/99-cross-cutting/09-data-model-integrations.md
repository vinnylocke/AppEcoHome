...
### Consumed by the Home dashboard (2026-07-02)

The **`home-overview`** edge function (Phase 2 of the new Home dashboard, `?view=home`) reads this graph server-side to build the Garden Overview grid's telemetry: the `latest_device_readings` RPC + `devices.battery_percent` feed each area's soil-sensor chip (moisture band / % + temp; grey when the reading is >24 h old), and `valve_events` (last 200) + `automation_valve_queue` pending/failed rows derive each area's valve chip state ("running" only inside the newest `turn_on`'s `duration_seconds` countdown with no newer `turn_off`; a failed queue row newer than the last event → "failed"; earliest pending `turn_on` → "Next water"). Failed `automation_runs` (24 h) and low-battery `devices` also feed the ranked attention row. All reads are home-bounded and service-role behind an explicit membership check. See [Home (Main Dashboard)](../02-dashboard/17-home-main.md) and the [Edge Functions Catalogue](./10-edge-functions-catalogue.md).
