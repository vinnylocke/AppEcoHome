...
| Tap-target audit (≥44×44px) | Wave 1–8 pushed `min-h-[40px]` on most action buttons. **dashboard-nav-tasks-tray Stage 3 (2026-07-21, B4)** raised the highest-traffic controls to the 44px floor via `pointer-coarse:` bumps: the task-row complete toggle (was 32px) + the bulk select-all (24px), the home hero's "Plan my day" + weather chips (36px), and the garden grid's add-location chip. A Lighthouse-driven re-pass for the long tail is still the right cadence. |
| Task-state colours on HC-aware tokens | **Stage 3 (B3):** TaskList's overdue / today / harvest card states + the meaning-carrying chips (overdue-since, late, harvested, plant, auto-watered, weather) moved off raw Tailwind palette (`bg-red-100` …) onto the `status-*` families (danger / caution / success / water), so they respond to the high-contrast override instead of silently ignoring it. The relative due label (B2) uses `text-status-danger-ink` for overdue, so it's no longer colour-only. |
