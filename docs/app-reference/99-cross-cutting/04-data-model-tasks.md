...
**Authoring.** `recurrence_kind` + `recurs_until` are set automatically at schedule-generation time from the plant lifecycle (`plantScheduleGenerator.buildBlueprintFromSchedule` + its mirror `PlantScheduleTab.getDatesForBlueprint`: perennial → repeats forever (`annual`); **biennial → exactly 2 windows** (`lifecycle_capped`, `recurs_until = firstWindow + 1yr`); annual / unknown → `once`). Users override via the **"Repeat every year"** checkbox + an optional **"Stop after N years"** number input (blank = forever) in the routine editors — `AddTaskModal` (which also backs BlueprintManager's edit, all four blueprint write paths) and `InstanceCareRoutine` (create path; the inline edit preserves the stored value). Both controls appear only when an `end_date` is set; clearing the end date resets recurrence to `once`. The **controls ↔ columns** mapping is the pure `src/lib/recurrence.ts` (`deriveRecurrence` / `yearsFromRecurrence`): "repeat for N years" = **N annual windows** (`recurs_until = start + (N−1) years`; N=1 → single window; forever → `annual`, no cap). `BlueprintManager`'s "Next:" preview rolls a recurring blueprint's window into the current occurrence via `projectAnnualWindows` so it never shows the expired one.
