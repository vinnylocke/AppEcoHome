...
| Run Automations | hourly (`0 * * * *`) | `run-automations` | Fire due watering automations. Cadence is hourly because the function matches automations by `scheduled_time` hour only; minute-level precision is handled by the valve-queue drain below. |
