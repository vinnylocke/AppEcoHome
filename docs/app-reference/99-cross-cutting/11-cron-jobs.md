...
| Plant Library Seed | daily (02:00 UTC) | `seed-plant-library` | Ask Gemini for 1000 plants in batches of 25, fetch a Wikipedia thumbnail each (cached via `plant_image_cache`), insert into `plant_library` with `valid = null`. Dedup via `scientific_name_key` unique index. Fire-and-forget background run; progress streams to `plant_library_runs`. |
| Plant Library Verify | daily (04:00 UTC) | `verify-plant-library` | Pick up to 2000 unverified rows, cross-check each against Wikipedia + GBIF, ask AI to compare under a tolerance-banded rubric. `valid = true` (matched) or amend the diverging fields and set `valid = false` with cited sources. See [Plant Library Admin](../07-management/08-plant-library-admin.md). |
