...
| `seed-plant-library` | Cron (daily 02:00 UTC) **+** admin manual trigger | Asks Gemini for N plants (batches of 25, cap 5000), fetches a free thumbnail per plant via `plant-image-search` (cached server-side via `plant_image_cache`), inserts into `public.plant_library` with `valid = null` and `ON CONFLICT (scientific_name_key) DO NOTHING`. Fire-and-forget: HTTP responds `{ run_id }` after creating a `plant_library_runs` row; work continues in `EdgeRuntime.waitUntil`. Run progress (inserted / skipped / failed) updates live on the run row. `verify_jwt = false` because pg_cron invokes without an auth header; service-role client is used internally for all writes. See [Plant Library Admin](../07-management/08-plant-library-admin.md). |
| `verify-plant-library` | Cron (daily 04:00 UTC) **+** admin manual trigger | Picks unverified rows (`verified_at IS NULL`), fetches Wikipedia summary + GBIF taxonomy match via `_shared/plantLibrarySources.ts`, asks Gemini to compare under a tolerance-banded rubric. `verdict='matched'` → `valid=true, verified_at=now()`. `verdict='amended'` → overwrite the diverging fields (`pickAllowedUpdates` keeps the update surgical), set `valid=false`, store cited sources `[{ url, title, source, licence, accessed_at }]`. When neither source returns anything usable we mark the row `valid=true` (default-pass) to avoid churn. Same fire-and-forget pattern as the seeder. Verifier prompt explicitly instructs the AI to synthesise corrected descriptions in our own voice (not paraphrase Wikipedia) so the CC BY-SA share-alike clause doesn't bite. See [Plant Library Admin](../07-management/08-plant-library-admin.md). |
