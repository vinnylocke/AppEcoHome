...
| `area-sensor-analysis` | AI Area Coach tab (Area Metrics modal) | **AI-tier only.** Cache-aware soil-sensor coaching for one area. Gathers the area's linked soil sensors + their current readings + 30-day history aggregates (`device_readings`), the plants in the area (`inventory_items` → `plants.soil_ph_min/max`), and the area's moisture-triggered automations (`automation_sensors` + `automation_actions`). Persona-aware Gemini call (`user_profiles.persona`) returns per-metric target ranges (moisture/EC/soil-temp) with status + meaning + recommendation, an automation review/suggestions, headline + summary. **Cached one-row-per-area in `area_ai_insights`**; regenerates only when a `device_reading` newer than `based_on_reading_at` arrives (live or manual) or `force`. `enforceRateLimit` caps regenerations; `logAiUsage` records spend. See [Location Manager](../03-garden-hub/03-location-manager.md). |
