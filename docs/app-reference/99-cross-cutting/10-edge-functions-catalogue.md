...
| `agent-chat` | on demand | Tool-aware extension of Plant Doctor chat. Routes text-only messages through Gemini in function-calling mode; 47 tools across read/auto (17, incl. `optimise_area_schedule`, `list_devices`, `list_automations`), safe-create (10), structural (7), destructive/bulk (9), automations (4: create/update/run/delete), gated by confirm cards. Tier-gated daily message quota via `check_ai_message_quota`. See [Agent Tools Catalogue](./35-agent-tools.md). |
