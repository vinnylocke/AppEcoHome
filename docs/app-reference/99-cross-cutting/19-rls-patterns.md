...
**Client `can()` is the only spatial-key guard — and now two surfaces rely on it.** The `locations` table's RLS gates only home *membership* (the canonical `home_members` subquery), not the `locations.create` / `locations.edit` / `locations.delete` keys. Those keys are enforced purely by `usePermissions().can(...)` at the call sites. As of the stats+locations redesign Stage 4b (2026-07-20) there are **two** such call sites sharing one DB path (`src/lib/locationMutations.ts`): LocationManager (`/management`) **and** the home garden grid's inline add/manage (`GardenOverviewGrid`'s `home-add-location-btn` + each card's `LocationManageMenu`). Both gate identically — a viewer sees no add button and no manage kebab; a member can create + edit but not delete — but a session token could still bypass either UI via direct PostgREST. `locationMutations.ts` is deliberately permission-agnostic (it returns the raw `{ error }`); the `can()` guard lives in the callers, so **any promotion of the `locations.*` keys to RLS must cover both surfaces at once.** See [Members & Permissions](../07-management/02-members-permissions.md) + [Home (Main Dashboard)](../02-dashboard/17-home-main.md).
