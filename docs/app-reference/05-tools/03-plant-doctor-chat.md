...
> - **The Rhozly Reply Template** (canonical copy: [docs/ai-chat-eval/reply-template.md](../../ai-chat-eval/reply-template.md), rendered on the eval report): bottom-line sentence → bold-labelled detail bullets → `🔎 Checked:` (data read) → `🔧 Ready to confirm:` (staged actions + dependent follow-ons) → one `→` next step. Bare "I need a quick confirmation" replies are banned — the server composes an informative fallback from the pending previews when the model stages cards without text, and a **phantom-🔧 guard** strips any `🔧` line from a turn with no pending card (the model can't claim something is staged when nothing is; logged as `phantom_action_line_stripped`).
> - **Round 2 (eval-driven):** defaults-not-interrogation (resolves areas/plants itself, stages an editable card with sensible cadence, ≤1 clarifying question); never stages a guessed id (a failed mutation preview is bounced back to the model to self-correct instead of staging a broken card); dependent chains (link-after-add, plants-after-plan, sow-after-packet) stage step A and announce step B for after confirmation; refinements of a just-staged action re-stage immediately; attention questions → `get_overdue_summary` answered in plants; optimise requests → `optimise_area_schedule`; `list_devices` now attaches each device's **latest reading** so sensor questions quote real values; no `search_plant_database` for general-knowledge facts.
> - **APP FACTS grounding (round 9):** the system prompt includes a maintained capability truth-table (`agent-chat/appFacts.ts`) — what Rhozly can do in plain words (photo ID, shared homes, native frost alerts, Ecowitt/eWeLink/webhook-only sensors, bulk paste-import…) and what it can't (CSV export, API, printing) — so capability questions are answered from facts, never guessed. **Keep appFacts.ts in sync when features ship.** A second forced-retry trigger grounds data claims: a reply asserting "your X is empty"-style facts with zero read tools that turn is re-asked with tool calling forced (`claimsUserData` in `actionIntent.ts`).
