...
- [x] [Calendar Tab](./02-dashboard/03-calendar-tab.md) — Month / Week views, drag-reschedule, ICS export. **#12: now the default tab of the [Calendar Section](./02-dashboard/19-calendar-section.md) at `/calendar`** (was the dashboard `?view=calendar` sub-tab)
- [x] [Weather Tab](./02-dashboard/04-weather-tab.md) — 7-day forecast, alerts, sunrise/sunset. **#12: now the Weather tab of the [Calendar Section](./02-dashboard/19-calendar-section.md) at `/calendar?tab=weather`** (was the dashboard `?view=weather` sub-tab)
...
- [~] [Quick Access Home](./02-dashboard/09-quick-access-home.md) — **ARCHIVED 2026-07-23** (RETIRED 2026-07-20, "one responsive home", `/quick` now redirects to `/dashboard`; the Quick Launcher customiser it also documented was removed outright 2026-07-23); the planting helper is kept at `/quick/calendar` via the dashboard's Today tile
...
- [x] [Calendar Section (CalendarHub)](./02-dashboard/19-calendar-section.md) — **#12 IA reorg** — the new top-level `/calendar` section: a `SegmentedTabs` hub over **Calendar · Weather · Routines**. Calendar + Weather left the Dashboard (their `?view=` pills + persistence deleted); Routines left the Planner + the standalone `/schedule` (now a redirect). Nav item in the Garden group; no Deck slot
...
- [x] [Ailment Library — ARCHIVED](./03-garden-hub/16-ailment-library.md) — page deleted (Hub v3 Stage F, 2026-07-22): `/ailment-library[?ailment=]` redirects to `/shed?tab=watchlist[&detail=]`; the field guide lives in the Ailments tab (search takeover + shared detail modal)
