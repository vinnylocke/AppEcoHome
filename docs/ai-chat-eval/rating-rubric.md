...
> **v1.3 (corrections, user-reported):** (a) the truth-table wrongly said each *location* has
> its own weather. **Weather is PER HOME** (weather_snapshots is keyed by home_id); a second
> site with different weather = a separate home (multiple homes are supported). RE19's
> run-35.0018 rating (5/5) was graded against the wrong truth — read with that caveat.
> (b) Plant-data provenance added: Rhozly's core library (90k+) is its OWN, mainly
...
- **Capability honesty is the accuracy bar.** Features that EXIST and should be answered
  ...
  journals/notes/photos per plant, MULTIPLE HOMES each with their own weather (weather is PER
  HOME — locations/areas within a home share its forecast, so a second different-weather site is
  a separate home; claiming per-location weather is an accuracy failure), shared homes with
...
- **Plant-data provenance (RE18):** the correct answer is that Rhozly's core plant library
  (90k+) is its OWN — mainly generated by the Gemini AI model and independently verified against
  Wikipedia/GBIF — with Perenual/Verdantly as separate premium API sources and Rhozly AI able to
  research plants on the fly. Claiming the core library is sourced *from* Perenual/Verdantly is
  an accuracy failure.
