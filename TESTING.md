...
### Unit tests — 1,517 tests across 139 files
...
| `home-main.spec.ts` | 8 | Home main dashboard, plain `/dashboard` (HOME-001..008): default landing + **4-tab switcher (Dashboard / Locations / Calendar / Weather — Phase 4.2 merged the old Overview tab into Home)**, overview grid renders seeded locations/areas, legacy `?view=dashboard` → home, legacy `?view=overview` **falls through to the merged Home view** (`home-main` visible), default quick-action tiles, density toggle persists `rhozly:home:density`, today's tasks "See all" → calendar; HOME-008 (Phase 2) mocks `home-overview` via `mockEdgeFunction` and asserts the sensor chip, valve chip ("Watering" running state) and `soil_dry` attention card. Page Object: `pages/HomeMainPage.ts`. `DashboardPage.goto()` now seeds `rhozly:home:density = "detailed"` and navigates to plain `/dashboard` — the classic-dashboard content lives behind the home's Detailed density, not a separate Overview URL |
