...
### Unit tests — 1,554 tests across 144 files
...
### E2E tests — 536 tests across 36 files (+ 13 isolation tests)
...
| `dashboard.spec.ts` | 45 | Dashboard sections, weather card, daily tasks, plant grid, nav links, pull-to-refresh; RHO-9 Week Ahead card gating (DASH-051/052). (DASH-050 "Total Tasks tile → calendar" RETIRED with the Garden Snapshot stat wall — stats+locations redesign Stage 2, 2026-07-20.) |
...
| `home-main.spec.ts` | 10 | Home main dashboard, plain `/dashboard` (HOME-001..008, HOME-013..014): default landing + **4-tab switcher (Dashboard / Locations / Calendar / Weather — Phase 4.2 merged the old Overview tab into Home)**, overview grid renders seeded locations/areas, legacy `?view=dashboard` → home, legacy `?view=overview` **falls through to the merged Home view** (`home-main` visible), default quick-action tiles, posture toggle persists `rhozly:home:density`, today's tasks "See all" → calendar; HOME-008 (Phase 2) seeds the workbench posture + mocks `home-overview` and asserts the sensor chip, valve chip ("Watering") and `soil_dry` attention card; **HOME-013** (Stage 4) the Porch surfaces the top attention item as the Next Best Action; **HOME-014** (stats+locations Stage 3) the compact today list exposes inline complete + Postpone + the task-board pill without leaving the home. Page Object: `pages/HomeMainPage.ts`. `DashboardPage.goto()` seeds `rhozly:home:density = "detailed"` and navigates to plain `/dashboard` — the classic-dashboard content lives behind the home's Workbench posture, not a separate Overview URL |
