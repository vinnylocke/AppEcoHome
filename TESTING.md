...
### Unit tests — 1,677 tests across 157 files
...
| `soilReadings.test.ts` | 8 | `buildSoilChips` — area-tile soil chips from `latest_soil_*` columns: empty→[], moisture→temp→ec order + labels, skip-missing-metric, rounding (moisture/EC int, temp 1dp), stale >24h / no-timestamp / unparseable, fresh at the 24h boundary (2026-07-23) |
...
### Edge function tests — Deno (1,036 tests across 93 files)
...
| `soilBehaviour.test.ts` | 8 | `_shared/soilProfile/behaviour.ts` — temp behaviour (daily max/min/diurnal swing, gates) + EC behaviour (mean, CV → stability, trend, zero-mean guard) for the device Soil-behaviour panel (2026-07-22) |
...
### E2E tests — 579 tests across 40 files (+ 13 isolation tests)
...
> `ailment-library.spec.ts` (Section 36) is 2 REDIRECT tests since Hub v3 Stage F (2026-07-22): the /ailment-library page died — AL-R1 (`/ailment-library` → `/shed?tab=watchlist`) and AL-R2 (`?ailment=900001` → `&detail=900001` opens the shared field-guide modal). The old browse/filter/watch/favourite coverage lives on in `watchlist.spec.ts` (WL-E1/E1b/E2 + the takeover specs); the library seeds (`16_ailment_library.sql`, ids 900001–900003, GLOBAL table seeded per-worker-idempotently) remain load-bearing.
...
| File | Tests | Coverage |
|------|-------|----------|
| `today-tasks-tray.spec.ts` | 6 | Global Today's-Tasks tray (Section 34, dashboard-nav-tasks-tray Stage 2) — header trigger opens the drawer from a non-home screen + close (TRAY-001), inline complete/postpone on rows (TRAY-002), quick-add opens QuickAddTaskModal (TRAY-003), board button → `?view=calendar` (TRAY-004), the Today/Completed tabs with the seeded completed-today task + inline undo (TRAY-005, 2026-07-22), focus-mode hides the trigger (TRAY-010) |
| `integrations-device.spec.ts` | 1 | Device detail modal (2026-07-22) — the seeded soil sensor's Soil-behaviour panel renders drainage / day-night temperature / EC indicators from the seeded `soil_moisture_profiles` row (IDD-001) |
| `stage4-discoverability.spec.ts` | 6 | Stage 4/5 discoverability (Section 35) — the Ailments Tools tile is gone (DISC-B5, removed 2026-07-22), Planner Routines tab renders BlueprintManager (DISC-B12), the no-op "Getting Started" account item is gone (DISC-B8), the Schedule header shows a live summary not "Operational Hub" (DISC-B15), the mobile Shelf lists Planner but drops the Deck's Home/Plants tabs (DISC-B7, updated 2026-07-22), Garden Reports routed at `/reports` via the Tools tile (DISC-B16) |
...
| `recurrence-cap.spec.ts` | 2 | Track B annual-recurrence authoring (RCUR-001..002): "Repeat every year" + "Stop after N years" controls reveal progressively (min=1, reset-on-clear) and save an annual routine with a year cap |
...
| `shed-crud.spec.ts` | 39 | Add plant (manual + AI), edit, archive, restore, delete, search/filter, detail drawer; + search overlay (SHED-TKO-001/002/003 — deep link, back, keyboard-safe top bar + Escape ladder) + Stage 3 landing (SHED-S3-001 escalation row, SHED-S3-002 persona browse chips); v3 feedback polish (2026-07-22): SHED-P1 "N past" chip, SHED-P2 visibility-law hidden hint; SHED-TKO-004 (2026-07-22) same-named library species select independently (`lib:{id}` selection keys); SHED-005 (2026-07-23) — Active is the smart-default chip when the garden has live plants, All surfaces inactive |
...
| `area-setup.spec.ts` | 22 | Location management, create location, create area, assign plant, delete flows; AREA-SOIL-001 (2026-07-23) — live soil-reading chips on the area tile (`buildSoilChips`, direct `latest_soil_*` write + cleanup) |
...
| `watchlist.spec.ts` | 34 | Ailment list, type filters, add manual/AI, detail modal tabs, archive/restore/delete; + search overlay (WL-TKO-001 open/back, WL-TKO-002 ?open=add-ailment deep link, WL-TKO-003 keyboard-safe input + row-tap → field-guide detail parity); v3 feedback polish (2026-07-22): rebrand to Binoculars/watchlist voice, card-kebab archive/delete, merged Watchlist chip, WL-P1 visibility-law hidden hint; WL-tab-smart-default (2026-07-23) — Active is the default chip when live-linked ailments exist |
| `layout.spec.ts` | 10 | Nav bar visibility, active route highlighting, responsive layout; NAV-012 (2026-07-22) — the Deck's Tasks slot opens the tray, the phone header trigger is hidden, Planner reachable via More → Shelf |
