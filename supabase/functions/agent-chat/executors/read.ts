...
// ─── get_daily_brief ───────────────────────────────────────────────────
// Reads the stored Daily Brief (Garden Brain Phase 2) — no AI call here; the
// brief was generated by the 04:30 cron (deterministic for all tiers, AI voice
// for Sage/Evergreen), so this tool is safe at sprout+.
export async function exec_get_daily_brief(ctx: ExecutorContext): Promise<ExecResult> {
  const todayIso = new Date().toISOString().split("T")[0];
  const { data: brief } = await ctx.db
    .from("daily_briefs")
    .select("brief_date, payload, generated_by")
    .eq("home_id", ctx.homeId)
    .lte("brief_date", todayIso)
  ...
  if (!brief) {
    return {
      payload: { brief: null },
      summary: "No Daily Brief has been generated for this home yet (it generates each morning).",
    };
  }
  const p = brief.payload as { summary?: string; items?: Array<{ title: string; reason: string }>; goodNews?: string[] };
  return {
    payload: { brief_date: brief.brief_date, ...p },
    summary: `Daily Brief for ${brief.brief_date}: ${p.summary ?? ""} Items: ${(p.items ?? [])
      .map((i) => i.title)
      .join(" · ")}${p.goodNews?.length ? ` Good news: ${p.goodNews.join(" ")}` : ""}`,
  };
}
...
export const READ_EXECUTORS: Record<string, Executor> = {
  ...
  get_daily_brief:       (ctx) => exec_get_daily_brief(ctx),
