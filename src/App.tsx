...
import { UserProfile, InventoryItem, GardenTask, Plant, WeatherData, Location, WeatherAlert } from './types';
...
import { LogIn, Leaf, Loader2, Sun, Droplets, Shovel, Calendar, Wheat, X, AlertTriangle } from 'lucide-react';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'calendar' | 'guides'>('dashboard');
  ...
  const locations = React.useMemo(() => {
    const activeMocks = MOCK_SCENARIOS.filter(s => s.enabled);
    const mockLocations = activeMocks.map(s => s.location).filter((l): l is Location => !!l);
  ...
  const weatherAlerts: WeatherAlert[] = React.useMemo(() => {
    ...
    locations.forEach(loc => {
      ...
      const processWarnings = (warnings: any, dayLabel: string, dateStr: string) => {
  ...
  useTaskNotifications(tasks, userProfile?.notificationIntervalHours || 8, weatherAlerts);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'user_profiles', filter: `uid=eq.${user.id}` }, (payload) => {
  ...
  useEffect(() => {
    ...
    const homeId = userProfile.homeId || user.id;
    ...
    const fetchData = async () => {
      ...
      if (invRes.data) setRealInventory(invRes.data.map(item => ({
        id: item.id,
      ...
      if (taskRes.data) setRealTasks(taskRes.data.map(task => ({
        id: task.id,
        title: task.title,
      ...
      if (locRes.data) setRealLocations(locRes.data.map(loc => ({
        id: loc.id,
        name: loc.name,
        address: loc.address,
        lat: loc.lat,
      ...
      if (plantRes.data) setPlants(plantRes.data.map(p => ({
        id: p.id,
        name: p.name,
    ...
    const invChannel = supabase.channel('inventory').on('postgres_changes', { event: '*', schema: 'public', table: 'inventory_items', filter: `home_id=eq.${homeId}` }, fetchData).subscribe();
    const taskChannel = supabase.channel('tasks').on('postgres_changes', { event: '*', schema: 'public', table: 'tasks', filter: `home_id=eq.${homeId}` }, fetchData).subscribe();
    const locChannel = supabase.channel('locations').on('postgres_changes', { event: '*', schema: 'public', table: 'locations', filter: `home_id=eq.${homeId}` }, fetchData).subscribe();
    const plantChannel = supabase.channel('plants').on('postgres_changes', { event: '*', schema: 'public', table: 'plants' }, fetchData).subscribe();
    ...
    const updateWeather = async () => {
      ...
      for (const loc of locations) {
        if (globalWeather) {
          ...
          const mockScenario = activeMocks.find(s => s.location?.id === loc.id);
          if (mockScenario && mockScenario.weather) {
            setWeatherMap(prev => ({ ...prev, [loc.id]: mockScenario.weather! }));
          } else {
            try {
  ...
  const handleRefreshWeather = async (locationId?: string) => {
    const locsToUpdate = locationId ? locations.filter(l => l.id === locationId) : locations;
    const activeMocks = MOCK_SCENARIOS.filter(s => s.enabled);
    
    await Promise.all(locsToUpdate.map(async (loc) => {
      try {
        const mockScenario = activeMocks.find(s => s.location.id === loc.id);
        if (mockScenario) {
          setWeatherMap(prev => ({ ...prev, [loc.id]: mockScenario.weather }));
  ...
  useEffect(() => {
    if (!user || tasks.length === 0 || Object.keys(weatherMap).length === 0) return;
    ...
    const autoUpdateTasks = async () => {
      ...
      const tasksToComplete = tasks.filter(task => {
        if (task.status !== 'Pending' || task.type !== 'Watering' || task.isVirtual) return false;
  ...
  useEffect(() => {
    ...
    const cleanupTasks = async () => {
      const tasksWithOldMessage = tasks.filter(task => 
        !task.isVirtual && 
        task.description?.includes('[Auto-completed: Rain expected at this location]')
      );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50 p-4">
        <div className="max-w-md w-full bg-white p-10 rounded-[2.5rem] shadow-2xl border border-red-100 text-center">
          ...
          <h1 className="text-2xl font-bold text-stone-900 mb-4">Configuration Missing</h1>
          <p className="text-stone-500 mb-8">
            Supabase URL or Anon Key is missing. Please add <code className="bg-stone-100 px-1.5 py-0.5 rounded text-red-600">VITE_SUPABASE_URL</code> and <code className="bg-stone-100 px-1.5 py-0.5 rounded text-red-600">VITE_SUPABASE_ANON_KEY</code> to your environment variables in AI Studio settings.
          </p>
          <button
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center p-4">
        ...
        >
          ...
          <h1 className="text-4xl font-bold text-emerald-900 mb-4 tracking-tight">EcoHome</h1>
          <p className="text-stone-500 mb-10 leading-relaxed">
            Your context-aware, AI-driven gardening companion. Grow smarter, not harder.
          </p>
          <button
            onClick={handleLogin}
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-stone-900">
                {plants.find(p => p.id === selectedShedItem.plantId)?.name || 'Plant Details'}
              </h2>
              <button
            ...
            {modalTab === 'search' ? (
              <div className="space-y-4 overflow-y-auto pr-2 flex-1">
                {(() => {
                  const plant = plants.find(p => p.id === selectedShedItem.plantId);
                  if (!plant) return <p className="text-sm text-stone-500">Care guide not available.</p>;
                  return (
              ...
              <div className="space-y-3 overflow-y-auto pr-2 flex-1">
                {inventory.filter(i => i.plantId === selectedShedItem.plantId && i.status === 'Planted').map(instance => (
                  <div key={instance.id} className="flex items-center justify-between p-4 bg-stone-50 rounded-2xl border border-stone-100">
                    <span className="text-sm font-medium text-stone-900">{getPlantDisplayName(instance)}</span>
                    <button 
                      onClick={() => {
          ...
          <div className="flex items-center gap-4 bg-white p-2 rounded-3xl border border-stone-100 shadow-sm self-start">
            <button
              ...
              className={cn(
                ...
                activeTab === 'dashboard' ? "bg-emerald-600 text-white shadow-lg shadow-emerald-200" : "text-stone-400 hover:text-stone-600"
              )}
            >
              Dashboard
            </button>
            <button
              ...
              className={cn(
                "px-6 py-2.5 rounded-2xl text-sm font-bold transition-all",
                activeTab === 'calendar' ? "bg-emerald-600 text-white shadow-lg shadow-emerald-200" : "text-stone-400 hover:text-stone-600"
              )}
            >
              Calendar
            </button>
            <button
              ...
              className={cn(
                "px-6 py-2.5 rounded-2xl text-sm font-bold transition-all",
                activeTab === 'guides' ? "bg-emerald-600 text-white shadow-lg shadow-emerald-200" : "text-stone-400 hover:text-stone-600"
              )}
            >
              Guides
            </button>
              ...
              >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <WeatherWidget 
                    ...
                    inventory={inventory}
                  />
                  <PlantDoctor mode={userProfile.mode} homeId={userProfile.homeId || user.id} inventory={inventory} />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="flex flex-col gap-8">
                    <LocationManager userId={user.id} homeId={userProfile.homeId || user.id} locations={locations} />
                  </div>
                </div>
                ...
                <InventoryManager 
                  userId={user.id} 
                  homeId={userProfile.homeId || user.id}
                  inventory={inventory} 
                  plants={plants} 
                  locations={locations} 
