  ...
  return (
    <div className="h-full flex flex-col bg-rhozly-bg animate-in slide-in-from-right-8 duration-500 relative z-40">
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 custom-scrollbar pb-24">
        ...
        {/* Wrap the phases in a div that disables them if not started */}
        <div
          className={`space-y-6 transition-all duration-500 ${!isStarted ? "opacity-40 grayscale-[30%] pointer-events-none select-none" : ""}`}
        >
            ...
            {isPhase2Done ? (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {localBlueprint.plant_manifest.map(
                    (plant: any, idx: number) => {
                      const isCreate =
                        plantMapping[idx] === "create" || !plantMapping[idx];
                      return (
                        <div
                          key={idx}
                          className="relative p-4 border border-emerald-100 rounded-2xl flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-emerald-50/30"
                        >
                          {!isPhase2Done && (
                            <button
                              onClick={() => handleDeletePlant(idx)}
                              className="absolute top-2 right-2 text-red-400 hover:text-red-600 hover:bg-red-50 p-2 rounded-xl transition-colors"
                          ...
                          <div className="flex items-start gap-3 flex-1 pt-2 sm:pt-0 pr-8">
                            {isCreate && (
                              <input
                                type="checkbox"
                                ...
                                className="mt-1 w-5 h-5 rounded border-emerald-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer shrink-0"
                              />
                            )}
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="space-y-2 mb-4 bg-white/50 p-3 rounded-xl border border-emerald-100/50">
                                <p className="text-xs text-gray-700 leading-snug">
                                  <span className="font-black text-emerald-900 block mb-0.5">
                                    Why it looks good:
                                  </span>
                                  {plant.aesthetic_reason ||
                                    "Selected for general aesthetic compatibility."}
                                </p>
                                <p className="text-xs text-gray-700 leading-snug">
                                  <span className="font-black text-emerald-900 block mb-0.5">
                                    Why it survives:
                                  ...
                                  {plant.horticultural_reason ||
                                    "Selected for appropriate climate and dimensions."}
                                </p>
                              </div>

                              <p className="text-[10px] font-bold text-emerald-800 bg-emerald-100 px-2 py-1 rounded-md inline-block">
                          ...
                          <div className="w-full sm:w-64 shrink-0 mt-2 sm:mt-0">
                            <select
                              ...
                              className={`w-full p-3 bg-white rounded-xl outline-none font-bold text-base sm:text-sm border focus:ring-2 transition-all ${
