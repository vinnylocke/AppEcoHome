...
import { UserProfile, UserMode, InventoryItem, GardenTask } from "../types";
...
export const PlantDoctor: React.FC<PlantDoctorProps> = ({
  ...
  const [loadingType, setLoadingType] = useState<
    "diagnose" | "identify" | "scheduling" | null
  >(null);
  ...
  const handleScheduleTasks = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Successfully scheduled ${scheduledCount} recovery tasks!`,
      );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-500">
              Upload a photo to diagnose issues
            </p>
          </div>
        </div>
        ...
        {activeTab === "doctor" ? (
          ...
          >
            {!userProfile.aiEnabled ? (
              // 🏥 THE "OFFLINE" MESSAGE
              <div className="flex flex-col items-center justify-center py-12 px-6 bg-stone-50 rounded-[2rem] border-2 border-dashed border-stone-200 text-center">
                <div className="w-20 h-20 bg-stone-200 text-stone-400 rounded-3xl flex items-center justify-center mb-6">
                  <div className="relative">
                    <Leaf size={40} />
                    <X
                      size={24}
                      className="absolute -top-1 -right-1 text-red-500 bg-white rounded-full border-2 border-white"
                    />
                  </div>
                </div>
                <h3 className="text-xl font-bold text-stone-900 mb-2">
                  Plant Doctor is Offline
                </h3>
                <p className="text-sm text-stone-500 max-w-xs mb-8">
                  AI features are currently disabled in your profile settings.
                  Enable them to diagnose plant issues and identify species.
                </p>
              </div>
            ) : (
              // 📸 THE ORIGINAL CAMERA/UPLOAD UI
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex flex-col gap-1">
                        ...
                        >
                          <option value="">
                            -- Select a plant from your garden --
                          </option>
                          {inventory.map((item) => (
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-col gap-2">
                          {diagnosis.tasks.map((task, idx) => (
                            <div
                              key={idx}
                              className="p-3 bg-white border border-stone-100 rounded-xl text-xs"
                            >
            ...
            {history.length === 0 ? (
              ...
              <div className="flex flex-col gap-4 max-h-[600px] overflow-y-auto pr-2">
                {history.map((item) => (
                  <div
                    key={item.id}
                    className="bg-stone-50 border border-stone-100 rounded-2xl overflow-hidden"
                  >
