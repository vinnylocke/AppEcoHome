...
import { Package, Plus, MapPin, Calendar, ArrowRight, Loader2, X, Trash2, Search, Info, RefreshCw, Sun, Droplets, Shovel, Wheat } from 'lucide-react';
import { InventoryItem, Plant, Location, GardenTask } from '../types';
import { supabase } from '../lib/supabase';
import { motion, AnimatePresence } from 'motion/react';
import { getPlantDisplayName } from '../utils/plantUtils';
import { generatePlantingSchedule, generateCareGuide, getCommonNames } from '../services/gemini';
import { searchPlantsDirect, searchPlantsByCommonName } from '../services/plantService';
...
export const InventoryManager: React.FC<InventoryManagerProps> = ({ userId, homeId, inventory, plants, locations, onViewPlantedInstance, onSelectShedItem }) => {
  ...
  const [environment, setEnvironment] = useState<'Indoors' | 'Outdoors'>('Outdoors');
  ...
  const [namingPlantData, setNamingPlantData] = useState<Partial<Plant> | null>(null);
  ...
  const [selectedShedItem, setSelectedShedItem] = useState<InventoryItem | null>(null);
  const [commonNamesMap, setCommonNamesMap] = useState<Record<string, string[]>>({});
  const [plantDetailsMap, setPlantDetailsMap] = useState<Record<string, Partial<Plant>>>({});
  const [fetchingCommonNames, setFetchingCommonNames] = useState<string | null>(null);
  const [fetchingPlantDetails, setFetchingPlantDetails] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (isAdding && modalTab === 'search' && !searchQuery && !hasSearchedExternal) {
  ...
  const handleSearch = async (type: 'direct' | 'common' | 'local') => {
    ...
    try {
      if (type === 'common') {
        // AI Search: perform both direct and AI-assisted scientific search
        const [direct, aiResults] = await Promise.all([
          searchPlantsDirect(query),
          searchPlantsByCommonName(query)
        ]);
        setDirectResults(direct);
        setAiResults(aiResults);
        setSearchTab(direct.length > 0 ? 'direct' : 'ai');
        setSearchResults(direct.length > 0 ? direct : aiResults);
        if (direct.length === 0 && aiResults.length === 0) {
          setError("No results found. Try another search.");
        }
      } else {
        // Direct search
        const results = await searchPlantsDirect(query);
        setSearchResults(results);
  ...
  const handleSelectPlantForNaming = async (plant: Partial<Plant>) => {
    setLoading(true);
    try {
        const names = await getCommonNames(plant.scientificName || plant.name || "");
  ...
  const handleExpandPlant = async (e: React.MouseEvent, plant: Partial<Plant>) => {
    e.stopPropagation();
      ...
      if (newGuide.careGuide) {
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            scientific_name: newGuide.scientificName || viewingPlant.scientificName
          })
        ...
        setViewingPlant({
          ...
          scientificName: newGuide.scientificName || viewingPlant.scientificName
        });
      }
    } catch (err) {
  ...
  const handlePlantNow = async () => {
    ...
    try {
      ...
      const derivedEnvironment = selectedArea.type === 'inside' ? 'Indoors' : 'Outdoors';
      ...
      try {
        ...
        for (const task of schedule) {
          ...
          tasksToInsert.push({
            ...
            description: task.description || `Scheduled ${taskType} for ${displayName}`,
            status: 'Pending',
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-500">Inventory of seeds and young plants</p>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-col gap-3">
        {shedItems.length === 0 ? (
          <div className="py-8 text-center bg-stone-50 rounded-2xl border border-stone-100">
            <p className="text-sm text-stone-400">Your shed is empty. Add some seeds!</p>
          </div>
        ) : (
          shedItems.map(item => (
            <motion.div
            ...
            >
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-stone-400 uppercase tracking-widest">Added {new Date(item.createdAt).toLocaleDateString()}</span>
              </div>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center">
                    {modalTab === 'search' ? <Search size={24} /> : <Package size={24} />}
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-stone-500">
                      {modalTab === 'search' ? 'Search library or use AI to find plants' : 'Manage items currently in your shed'}
                    </p>
                  </div>
                </div>
                <button 
              ...
              <div className="flex items-center gap-2 mb-6 p-1 bg-stone-100 rounded-2xl">
                <button
                  ...
                  className={cn(
                    ...
                    modalTab === 'search' ? "bg-white text-emerald-600 shadow-sm" : "text-stone-500 hover:text-stone-700"
                  )}
                >
                  Add New
                </button>
                <button
                  ...
                  className={cn(
                    "flex-1 py-2 text-xs font-bold rounded-xl transition-all",
                    modalTab === 'inventory' ? "bg-white text-emerald-600 shadow-sm" : "text-stone-500 hover:text-stone-700"
                  )}
                >
              ...
              {modalTab === 'search' ? (
                <div className="flex flex-col gap-4">
                  ...
                  {hasSearchedExternal && (directResults.length > 0 || aiResults.length > 0) && (
                    <div className="flex items-center gap-2 p-1 bg-stone-100 rounded-2xl">
                      <button
                        ...
                        className={cn(
                          ...
                          searchTab === 'direct' ? "bg-white text-emerald-600 shadow-sm" : "text-stone-500 hover:text-stone-700"
                        )}
                      >
                      ...
                      <button
                        ...
                        className={cn(
                          "flex-1 py-2 text-[10px] font-bold rounded-xl transition-all uppercase tracking-wider",
                          searchTab === 'ai' ? "bg-white text-emerald-600 shadow-sm" : "text-stone-500 hover:text-stone-700"
                        )}
                      >
                  ...
                  {searchResults.length > 0 && (
                    <div className="space-y-2 max-h-64 overflow-y-auto p-1">
                      <h4 className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-2">
                        {hasSearchedExternal ? (searchTab === 'direct' ? 'Direct Results' : 'AI-Assisted Results') : 'Plant Library'}
                      </h4>
                            ...
                            <div className="w-full p-4 text-left bg-stone-50 border border-stone-100 rounded-2xl hover:border-emerald-200 transition-all">
                              <div className="flex items-center justify-between">
                                <div 
                                  ...
                                  onClick={() => handleSelectPlantForNaming(plant)}
                                >
                                  <span className="text-sm font-bold text-stone-900 group-hover:text-emerald-700">{plant.name}</span>
                                  {plant.scientificName && <p className="text-[10px] text-stone-400 italic">{plant.scientificName}</p>}
                                </div>
                                <div className="flex items-center gap-2">
                                  {plant.id && (
                                    <button
                                  ...
                                  <button
                                    ...
                                    className={cn(
                                      ...
                                      isExpanded ? "bg-emerald-100 text-emerald-600" : "text-stone-300 hover:text-emerald-500"
                                    )}
                                  >
                                    <Plus size={16} className={cn("transition-transform", isExpanded && "rotate-45")} />
                                  </button>
                                </div>
                              </div>
                                ...
                                {isExpanded && (
                                  ...
                                  >
                                    <div className="pt-4 mt-4 border-t border-stone-100">
                                      <h5 className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-2">Common Names</h5>
                                      {fetchingCommonNames === plantId ? (
                                      ...
                                      ) : commonNamesMap[plantId] && commonNamesMap[plantId].length > 0 ? (
                                        <div className="flex flex-wrap gap-1.5">
                                          {commonNamesMap[plantId].map(name => (
                                            <button
                                              key={name}
                                              onClick={() => {
                                                setNamingPlantData({ ...plant, name });
                                                setCommonNames(commonNamesMap[plantId]);
                                        ...
                                        <p className="text-[10px] text-stone-400 italic">No common names found.</p>
                                      )}

                                      {plantDetailsMap[plantId] && plantDetailsMap[plantId].careGuide && (
                                        <div className="mt-4 p-3 bg-stone-100/50 rounded-xl border border-stone-100">
                                          <h5 className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-2">Care Guide</h5>
                                          <div className="grid grid-cols-2 gap-2">
                                            <div className="flex items-center gap-2">
                                              <Sun size={12} className="text-amber-500" />
                                              <span className="text-[10px] text-stone-600">{plantDetailsMap[plantId].careGuide!.sun}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                              <Droplets size={12} className="text-blue-500" />
                                              <span className="text-[10px] text-stone-600">{plantDetailsMap[plantId].careGuide!.water}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                              <Shovel size={12} className="text-stone-500" />
                                              <span className="text-[10px] text-stone-600">{plantDetailsMap[plantId].careGuide!.soil}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                              <Calendar size={12} className="text-emerald-500" />
                                              <span className="text-[10px] text-stone-600">{plantDetailsMap[plantId].careGuide!.plantingMonth}</span>
                                            </div>
                                          </div>
                                        </div>
                                      )}
      ...
      <AnimatePresence>
        {isNamingPlant && namingPlantData && (
          <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            ...
            >
              <h3 className="text-xl font-bold text-stone-900 mb-4">Name your {namingPlantData.name}</h3>
              <p className="text-sm text-stone-500 mb-6">Select a common name or enter your own to add it to your shed.</p>
              ...
              <div className="flex gap-2">
                <button
                  onClick={async () => {
                    const displayName = selectedDisplayName || customDisplayName || namingPlantData.name || "Plant";
                  ...
                  disabled={(!selectedDisplayName && !customDisplayName) || loading}
                ...
                >
                  {loading ? <Loader2 className="animate-spin" size={18} /> : 'Add to Shed'}
                </button>
                <button
        ...
        {viewingPlant && (
          <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-2xl font-bold text-stone-900">{viewingPlant.name}</h3>
                  {viewingPlant.scientificName && <p className="text-sm text-stone-400 italic">{viewingPlant.scientificName}</p>}
                </div>
                <div className="flex items-center gap-2">
                  <button
                  ...
                  <button onClick={() => setViewingPlant(null)} className="p-2 hover:bg-stone-100 rounded-full">
                    <X size={20} />
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div className="p-4 bg-amber-50 rounded-2xl flex flex-col gap-2">
                  ...
                  <p className="text-sm text-amber-800">{viewingPlant.careGuide.sun}</p>
                </div>
                <div className="p-4 bg-blue-50 rounded-2xl flex flex-col gap-2">
                  ...
                  <p className="text-sm text-blue-800">{viewingPlant.careGuide.water}</p>
                </div>
                <div className="p-4 bg-stone-100 rounded-2xl flex flex-col gap-2">
                  ...
                  <p className="text-sm text-stone-800">{viewingPlant.careGuide.soil}</p>
                </div>
                <div className="p-4 bg-emerald-50 rounded-2xl flex flex-col gap-2">
                  ...
                  <p className="text-sm text-emerald-800">{viewingPlant.careGuide.plantingMonth}</p>
                </div>
                {viewingPlant.careGuide.harvestMonth && (
                  <div className="p-4 bg-orange-50 rounded-2xl flex flex-col gap-2 col-span-2 sm:col-span-4">
                    ...
                    <p className="text-sm text-orange-800">{viewingPlant.careGuide.harvestMonth}</p>
                  </div>
                )}
              </div>
      ...
      <AnimatePresence>
        {plantingItem && (
          <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-stone-900">Planting {getPlantDisplayName(plantingItem)}</h3>
                <button onClick={() => setPlantingItem(null)} className="p-2 hover:bg-stone-100 rounded-full">
                  <X size={20} />
                </button>
              </div>
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-stone-600 uppercase tracking-wider ml-1">Identifier (Optional)</label>
                  <input
                    type="text"
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-stone-600 uppercase tracking-wider ml-1">Location Profile</label>
                  <select
                    value={locationId}
                    onChange={(e) => {
                ...
                {locationId && (
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-semibold text-stone-600 uppercase tracking-wider ml-1">Area</label>
                    {locations.find(l => l.id === locationId)?.areas && locations.find(l => l.id === locationId)!.areas!.length > 0 ? (
                      <select
                        value={areaId}
                      ...
                      >
                        ...
                        {locations.find(l => l.id === locationId)?.areas?.map(area => (
                          <option key={area.id} value={area.id}>{area.name} ({area.type === 'inside' ? 'Indoors' : 'Outdoors'})</option>
                        ))}
                      </select>
                    ) : (
                      <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl text-xs text-amber-700 flex items-center gap-2">
                        <Info size={14} />
                        Please add areas to this location in the Location Manager first.
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-stone-600 uppercase tracking-wider ml-1">Date Planted</label>
                ...
                >
                  {loading ? <Loader2 className="animate-spin" size={20} /> : 'Confirm Planting'}
                </button>
              </div>
            </motion.div>
          </div>
