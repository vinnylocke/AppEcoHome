...
export const PlantDetailsModal: React.FC<PlantDetailsModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "care" | "history" | "logs" | "guides" | "yield"
  >("care");
  ...
  const deduplicatedTasks = itemTasks.reduce((acc: GardenTask[], current) => {
    if (current.status === "Completed") {
      ...
      const existingPendingIndex = acc.findIndex(
        (item) =>
          item.inventoryItemId === current.inventoryItemId &&
          item.type === current.type &&
      ...
      if (newGuide.careGuide) {
        ...
        if (!plantId) {
          ...
          const { data: existingPlants, error: fetchError } = await supabase
            ...
            .select("id")
            .or(
              `name.eq."${newGuide.name || plantName}",scientific_name.eq."${newGuide.scientificName || ""}"`,
            )
            .limit(1);
  ...
  const handleAnalyseYield = async () => {
    ...
    const totalActual = (item.yieldData.harvests || []).reduce(
      (sum, h) => sum + h.amount,
      0,
    );
    try {
      const analysis = await analyseYieldGap(
        item,
        plant,
        item.yieldData.predictedYield,
        totalActual,
        weather,
      );
  ...
  const handleDelete = async () => {
    try {
      ...
      // 2. NOW delete the inventory item
          ...
          {activeTab === "history" && (
            <div className="flex flex-col gap-4">
              {sortedTasks.length === 0 ? (
                <p className="text-stone-400 text-center py-8">
                  No tasks recorded for this plant.
                </p>
              ) : (
                sortedTasks.map((task) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center ${task.status === "Completed" ? "bg-emerald-50 text-emerald-600" : "bg-stone-50 text-stone-400"}`}
                      >
                        {getTaskIcon(task.type)}
                      </div>
                      <div>
                        <h4
                          className={`font-bold ${task.status === "Completed" ? "text-stone-900" : "text-stone-500"}`}
                        >
                    ...
                    <span
                      className={`text-xs font-bold uppercase tracking-wider px-2 py-1 rounded-lg ${task.status === "Completed" ? "bg-emerald-100 text-emerald-700" : "bg-stone-100 text-stone-500"}`}
                    >
                      {task.status}
                    </span>
                  </div>
                ...
                <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                  ...
                  <div className="text-3xl font-bold text-emerald-900 mb-2">
                    ...
                    <span className="text-lg font-medium">
                      {item.yieldData.predictedUnit}
                    </span>
                  </div>
                  <p className="text-sm text-emerald-800 italic">
                    "{item.yieldData.predictionReasoning}"
                  </p>
                </div>
              ) : (
                <div className="p-8 bg-stone-50 rounded-2xl border border-dashed border-stone-200 text-center">
                  <p className="text-stone-500 mb-4">
                    No prediction yet. Use AI to estimate your harvest.
                  </p>
                </div>
              )}
                ...
                <div className="flex flex-col gap-2">
                  {(item.yieldData?.harvests || []).length > 0 ? (
                    [...(item.yieldData?.harvests || [])].reverse().map((h) => (
                      <div
                        key={h.id}
                        className="flex items-center justify-between p-3 bg-white rounded-xl border border-stone-100 shadow-sm"
                      >
                        <div className="flex items-center gap-3">
              ...
              {item.yieldData?.predictedYield &&
                (item.yieldData.harvests || []).length > 0 && (
                  <div className="pt-6 border-t border-stone-100">
                    ...
                    {!yieldAnalysis &&
                      (item.yieldData.harvests || []).reduce(
                        (sum, h) => sum + h.amount,
                        0,
                      ) < item.yieldData.predictedYield && (
                        <p className="text-xs text-stone-500 italic">
                          Your actual yield is lower than predicted. Use AI to
                          understand why.
                        </p>
                      )}
                  </div>
            ...
            <div className="flex flex-col gap-6">
              ...
              {loadingGuides ? (
                    ...
                    <div
                      ...
                      onClick={() => {
                        window.dispatchEvent(
                          new CustomEvent("navigate-to-guide", {
                            detail: guide.id,
                          }),
                        );
                        onClose();
                ...
                <div className="text-center py-12 bg-stone-50 rounded-3xl border border-dashed border-stone-200">
                  ...
                  <p className="text-stone-500 font-medium">
                    No specific guides for this plant yet.
                  </p>
                  <button
                    onClick={() => {
                      window.dispatchEvent(
