...
export async function fetchWeather(
  ...
  if (response.status === 429) {
    // If rate limited, throw a specific error or wait
  ...
  for (
    let i = currentIndex;
    i < currentIndex + 12 && i < hourly.time.length;
    i++
  ) {
    forecast.push({
      date: hourly.time[i],
  ...
  const next24hPrecip = hourly.precipitation.slice(
    currentIndex,
    currentIndex + 24,
  );
  ...
  const calculateWarnings = (
    ...
    const formatTimeRange = (indices: number[]) => {
      ...
      const endStr = times[indices[indices.length - 1]]
        .split("T")[1]
        .slice(0, 5);
    ...
    return {
      frost: {
        active: frostIndices.length > 0,
        timePeriod: formatTimeRange(frostIndices),
      },
      heat: {
        active: heatIndices.length > 0,
        timePeriod: formatTimeRange(heatIndices),
      },
      wind: {
      ...
      rain: {
        active: rainIndices.length > 0,
        timePeriod: formatTimeRange(rainIndices),
        amount: totalRain,
      },
    };
  };

  const getWindSeverity = (
    speed: number,
  ): "Low to Moderate" | "Moderate to Strong" | "High" | "Extreme" => {
  ...
  const getWindDescription = (speed: number) => {
    ...
    if (speed >= 89)
      return "Storm. Trees are uprooted; considerable structural damage.";
    if (speed >= 75) return "Strong Gale. Slight structural damage occurs.";
    if (speed >= 62)
      return "Gale. Twigs break off trees; walking is generally impeded.";
    if (speed >= 50)
      return "Near Gale. Whole trees in motion; walking is inconvenient.";
    if (speed >= 39)
      return "Strong Breeze. Large branches in motion; whistling heard.";
  ...
  const todayIndices = hourly.time.reduce(
    (acc: number[], t: string, i: number) => {
  ...
  const tomorrowDate = new Date(
    new Date(currentDateStr).getTime() + 24 * 60 * 60 * 1000,
  )
    .toISOString()
    .split("T")[0];
  const tomorrowIndices = hourly.time.reduce(
    (acc: number[], t: string, i: number) => {
